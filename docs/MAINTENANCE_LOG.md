...
- Source: PR #170 review, PR #172 auto-create update, PR #173 auto-merge re-evaluation update, PR #177 manual PR plus successful auto-merge, and Codex no-origin verification on 2026-06-22.
- Status: Open for auto-create only; green docs-only auto-merge is proven for manually created `codex/*` -> `phase-13` PRs after PR #177.
- Context: Phase 13 automation retarget is active-phase generalized: policy active_base is phase-13, auto-create reads policy from requested/resolved BASE_BRANCH, push-triggered auto-create is intended for codex/** branch pushes, and auto-merge reads policy/classifier from resolved PR BASE_BRANCH. PR #177 was manually created and then auto-merged successfully, proving green docs-only auto-merge after a confirmed GitHub PR URL/number exists. Pushed-branch auto-create remains unresolved/unproven: the controlled verification stopped before editing because the Codex Cloud checkout had no `origin` remote (`git remote -v` produced no output; `git remote get-url origin` returned `No such remote 'origin'`), so no push was attempted, no pushed codex/* branch reached GitHub, and no push event could trigger auto-create. This is a Codex no-origin/no-push capability blocker, not proof of GitHub Actions auto-create workflow failure.
- Suggested action: keep Codex Cloud platform/manual `Create PR` as the default PR path, then allow GitHub Actions auto-merge for eligible green PRs only after a confirmed GitHub PR URL/number exists. Treat push-triggered auto-create as fallback-only for environments with a real authenticated `codex/*` push path. Pause further auto-create workflow edits until such a path exists and provides trigger evidence. Do not add token/PAT remote injection to normal Codex prompts or docs.
- Guardrails: workflow validation/docs only; do not change runtime files, Worker, OAuth/session, sync/conflict, schema/header mapping, CSV import/export, deployment routes, hosting, framework/build-step configuration, app-shell fingerprints, workflow permissions, or token/PAT handling in normal prompts.
- Acceptance: either a real pushed codex/* branch reaches GitHub and automatically creates a guarded phase-13 PR, or the platform/manual PR fallback remains documented; green docs-only manual/platform PRs can auto-merge only after all gates pass and a confirmed PR URL/number exists.
...
- Context: Current Codex Cloud diagnostics show no normal authenticated push path (`git remote -v` empty, no `origin`, no remote Git config, no visible credential helper/askpass/insteadOf/authenticated GitHub push path, and no `gh`), so platform/manual `Create PR` is the default PR path. Workflow inspection found no `upload-artifact`, `download-artifact`, or `retention-days` usage in `.github/workflows/**`, so there are no workflow-generated artifact-retention settings to clean up. Legacy or superseded candidates are `.github/workflows/ai-pr-create.yml` and `.github/workflows/ai-pr-merge.yml` because they are named prototypes and overlap with the newer guarded `.github/workflows/ai-pr-auto-create.yml` and `.github/workflows/ai-pr-auto-merge.yml`; `.github/workflows/agent-low-risk-guard.yml` targets `main` and appears tied to an older low-risk lane; `.github/workflows/validate.yml` still targets `phase-11`, which is historical; `.github/workflows/ai-cleanup-report.yml` is report-only but still hardcodes `phase-12`; `docs/agent-auto-pr-lane-handoff.md` documents an old `agent-auto-pr-lane`/`main` workflow; `scripts/wg13-readonly-phase13.sh` is optional legacy local helper tooling.
- Suggested action: keep `.github/workflows/ai-pr-auto-merge.yml` because it remains the active eligible-green-PR auto-merge path after confirmed PR creation. Defer `.github/workflows/ai-pr-auto-create.yml` rather than deleting it because it remains fallback-only for environments with a real authenticated `codex/*` push path. Consider removing or archiving `.github/workflows/ai-pr-create.yml`, `.github/workflows/ai-pr-merge.yml`, `.github/workflows/agent-low-risk-guard.yml`, `docs/agent-auto-pr-lane-handoff.md`, and `scripts/wg13-readonly-phase13.sh` in a separate explicitly approved cleanup PR. Treat `.github/workflows/validate.yml` and `.github/workflows/ai-cleanup-report.yml` as risky cleanup candidates because changing branch targets or replacing validation/cleanup behavior affects workflow governance and should be explicitly approved.
...
- Acceptance: a follow-up cleanup PR either removes/archives the approved legacy docs/scripts/workflows or records why each item is intentionally retained; active docs continue to state that Codex Cloud platform/manual `Create PR` is the default PR path and that push-triggered auto-create is fallback-only.
