...
- Suggested action: keep Codex platform PR creation or manual PR creation as the operating fallback, then allow GitHub Actions auto-merge for eligible green PRs only after a confirmed GitHub PR URL/number exists. Pause further auto-create workflow edits until a real pushed codex/* branch can reach GitHub and provide trigger evidence. Do not add token/PAT remote injection to normal Codex prompts or docs.
