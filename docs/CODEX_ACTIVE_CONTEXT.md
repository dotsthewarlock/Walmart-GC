...
- `.github/workflows/ai-pr-auto-create.yml` creates guarded `codex/*` -> `phase-13` PRs automatically on pushed `codex/**` branches, with a guarded branch-registration `create` fallback and `workflow_dispatch` preserved as a manual fallback, only when `auto_pr.enabled` is true and the requested/resolved base matches both policy `active_base` and the active context.
