...
- Before any task requiring PR creation or auto-merge, confirm the intended PR base is `phase-11`, the intended PR head is a `codex/*` branch, and a GitHub PR creation mechanism is available. For local/CLI workflows, `git remote get-url origin` may be used as a push preflight and should point to `dotsthewarlock/Walmart-GC`; do not require local `origin` as a universal Codex Cloud preflight because Codex Cloud may use platform-managed PR creation. If PR capability is unavailable, stop before implementation and report `blocked: PR creation unavailable` unless local-only / PR-ready output is explicitly approved.
- Distinguish Codex Cloud workspace commits and PR metadata from GitHub PRs. A commit reported by Codex Cloud is not enough to prove a branch was pushed to GitHub, and “Created PR metadata” is not a confirmed PR. A real PR exists only when Codex/GitHub reports a PR URL or PR number. If the Codex Cloud UI shows `Create PR`, treat the task as PR-ready but not PR-created.
- Once a real GitHub PR URL/number exists, Codex reports must use that confirmed PR state and actual PR head branch, and must not retain stale “PR-ready”, “local-only”, or workspace-only wording.
- PR creation is allowed when local `work` is only internal staging, changes are pushed or platform-submitted to a proper `codex/*` GitHub head branch, the PR base is `phase-11`, and a confirmed GitHub PR URL/number exists before reporting that a PR exists.
- Auto-merge is allowed only when an actual GitHub PR URL/number is confirmed, the PR base is `phase-11`, the PR head is the intended `codex/*` branch, checks/validation pass, no conflicts exist, confidence is >=80%, and no restricted-risk gates are triggered. No confirmed PR URL/number means no auto-merge.
