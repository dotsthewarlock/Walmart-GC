...
- Before any task requiring PR creation or auto-merge, run PR capability preflight before editing files: confirm `git remote get-url origin` works and points to `dotsthewarlock/Walmart-GC`, the intended PR base is `phase-11`, the intended PR head is a `codex/*` branch, and GitHub auth / PR creation mechanism is available. If any preflight check fails, stop before implementation and report `blocked: PR creation unavailable`. Do not leave a local-only patch for PR-required tasks unless the user explicitly approves local-only output.
- PR creation is allowed when local `work` is only internal staging, changes are pushed to a proper `codex/*` GitHub head branch, and the PR base is `phase-11`. If a UI/report says “Created PR” but no PR URL or PR number exists, treat PR creation as incomplete and report that status instead of treating the PR as available.
- Auto-merge is allowed only when an actual GitHub PR exists, the PR base is `phase-11`, the PR head is the intended `codex/*` branch, checks/validation pass, no conflicts exist, confidence is >=80%, and no restricted-risk gates are triggered.
