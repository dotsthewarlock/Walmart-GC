...
- `.github/workflows/ai-pr-auto-create.yml` is fallback-only for environments that can actually push a real `codex/**` branch to GitHub, with a guarded branch-registration `create` fallback and `workflow_dispatch` preserved as a manual fallback, only when `auto_pr.enabled` is true and the requested/resolved base matches both policy `active_base` and the active context. Stop spending workflow-fix cycles on push-triggered auto-create until an official authenticated Codex Cloud push path is visible.
- `.github/workflows/ai-pr-auto-merge.yml` handles guarded green-risk auto-merge only when policy auto-merge knobs are enabled and all workflow gates pass; it evaluates pull request changes and re-evaluates after completed validation signals via `workflow_run` and `check_suite` events that resolve exactly one open PR by head branch and head SHA.
...
- Lane 0 plus handoff update may produce a docs-only diff, normally `docs/AI_HANDOFF.md`; if that diff is committed in Codex Cloud, use platform/manual `Create PR` by default to create a guarded `codex/*` -> `phase-13` PR. Push-triggered auto-create is fallback-only for environments with a real authenticated push path.
